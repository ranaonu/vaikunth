                 ...
                 >
                   <thead>
                     <tr>
                       ...
                       <th><strong>Therepist Name</strong></th>
                   ...
                   <tbody>
                    @if(count($bookingSessions)>0)
                     ...
                     <tr> 
                        ...
                        <td>{{$booking['therepist_detail']['name']}}</td>
                        <td>{{date('d/m/Y',strtotime($booking['schedule_date']))}} </td>
                        <td>{{($booking['scheduled_time'])}} </td>
                        <td>
                          @if (strtotime($booking['schedule_date']) >= strtotime(date('Y-m-d')))
                        ...
                        <td>
                          @if ($booking['complete']<3)  
                          <a href="{{url('/')}}/admin/edit/session/{{base64_encode($booking['id'])}}">
                            <button type="button" class="btn btn-info btn-sm text-white" style="margin-bottom: 5px;">
                              Edit
                            </button> </a>
                          @endif
                          @if ($booking['complete']<2)  
                          <a href="{{url('/')}}/admin/view/payment/{{base64_encode($booking['id'])}}"><button type="button" class="btn btn-cyan btn-sm text-white" style="margin-bottom: 5px;">
                              View Payment
                            </button> </a>
                          @endif
                          ...
                          @if ($booking['complete']<3)
                            ...
                            @if (strtotime(date('Y-m-d')) > strtotime($booking['schedule_date']) && $booking['reschedule']==0)

                              <a href="{{url('/')}}/admin/reschedule/booking/{{base64_encode($booking['id'])}}"><button type="button" class="btn btn-cyan btn-sm text-white" style="margin-bottom: 5px;">
                                  Reschedule
                                </button> 
                              </a>
                            @endif
                          ...
                          @if ($booking['complete']>1)
                          <a href="{{url('/')}}/admin/view/booking/{{base64_encode($booking['id'])}}"><button type="button" class="btn btn-cyan btn-sm text-white" style="margin-bottom: 5px;">
                              View
                            </button>
                          </a>
                          @endif
